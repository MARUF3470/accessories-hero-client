...
const Wishlist = () => {
    ...
    const handleDelete = (id) => {
        ...
        toast.error('Removed')
        const updatedproducts = JSON.parse(localStorage.getItem('myWishlist'));
            ...
            <div>
                {products?.length ? <div className="overflow-x-auto w-11/12 mx-auto my-4">
                    <table className="table w-full">
                        ...
                        <tbody>
                            {newProducts.map(product => <WishlistItem key={product.id} product={product} handleDelete={handleDelete} ></WishlistItem>)}
                        </tbody>
                    </table>
